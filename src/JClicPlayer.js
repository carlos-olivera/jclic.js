...
define([
  ...
  "./report/Reporter"
], function (JSZip, ScriptJS, i18n, Utils, AWT, PlayerHistory, ActiveMediaBag, Skin,
  EventSounds, JClicProject, JumpInfo, ActiveBoxContent, Reporter) {
    ...
    class JClicPlayer extends AWT.Container {
      ...
      /**
       * Loads and parses the specified project from five different sources:
       * - An URL pointing to a `.jclic` file
       * - A file path pointing to a `.jclic` file. In this case, the equivalent `.jclic.js` file will be loaded.
       * - An URL or file path pointing to a `.jclic.zip` file
       * - An URL or file path pointing to a `.scorm.zip` file
       * - An URL pointing to a `project.json` file
       * @param {string} project - A project path or URL.
       * @returns {Promise} - Resolves with an object of type {@link JClicProject}.
       */
      loadProject(project) {
          ...
          return fetch(fullPath)
            ...
            .then(json => {
              ...
              if (Utils.endsWith(json['mainFile'], '.jclic'))
                ...
                return this.loadProject(Utils.getPath(Utils.getBasePath(fullPath), json['mainFile']));
              else
        ...
        if (fullPath.endsWith('.zip')) {
          ...
          return fetch(fullPath)
            ...
            .then(zip => {
              ...
              if (this.zip.files['project.json'])
                return this.zip.files['project.json'].async('string')
                  .then(content => {
                    ...
                    if (Utils.endsWith(json['mainFile'], '.jclic'))
                      // Load project's main file
                      return this.loadProject(Utils.getPath(this.zip.zipBasePath, json['mainFile']));
                    else
                      throw `Invalid or null "mainFile" specified in ${fullPath} - "project.json"`;
                  });
              ...
              if (fileName)
                return this.loadProject(Utils.getPath(this.zip.zipBasePath, fileName));
              else
        ...
        if (this.localFS && window.JClicObject && !window.JClicObject.projectFiles[fullPath])
          return new Promise((resolve, _reject) => ScriptJS(`${fullPath}.js`, () => resolve(this.loadProject(project))));
        ...
        // Special case for local file systems (using `file` protocol)
          ...
          if (window.JClicObject && window.JClicObject.projectFiles[fullPath])
            ...
            return Promise.reject(`Unable to load: ${fullPath}.js`);
        }
        return this.processProjectFile(fp, fullPath);
      }

      /**
       * Load and parse a `.jclic` XML file
       * @param {string} fp - Relative or absolute path to the XML file
       * @param {string} fullPath - Full path, needed to build the {@link JClicProject} object
       * @returns {Promise} - Resolves with an object of type {@link JClicProject}
       */
      ...
      /**
       * Builds a new {@link JClicProject} and configures it with the content of the provided XML text
       * @param {string} xml - The project content, encoded as an XML string
       * @param {string} fullPath - Full path to the project, needed to build the {@link JClicProject} object
       * @returns {JClicProject} - The resulting project
       */
      ...
      /**
       * Requests all media components related to a JClic project and waits until all of them are realized
       * When the time specified in `this.options.maxWaitTime` is reached, the promise resolves without any
       * error (the downloading of media continues in background), but a warning is logged to the console.
       * @param {JClicProject} prj - The JClic project
       * @param {number=} interval - The time interval between media checking loops. Defaults to a half second.
       * @returns {Promise} - Always resolves with `prj`
       */
      waitForMedia(prj, interval = 500) {
        ...
        return new Promise((resolve, _reject) => {
          const checkMedia = window.setInterval(() => {
            ...
            const timeout = ++loopCounter > this.options.maxWaitTime / interval;
            const waitingObjects = prj.mediaBag.countWaitingElements();
            if (waitingObjects === -1 || timeout) {
              window.clearInterval(checkMedia);
              this.setProgress(-1);
              if (timeout)
                Utils.log(`warning`, `Timeout while loading media. Some components could not be fuly loaded!`);
              resolve(prj);
            }
          }, interval);
        });
