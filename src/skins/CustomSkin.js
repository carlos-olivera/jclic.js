...
define([
  ...
  class CustomSkin extends Skin {
    ...
    constructor(ps, name = null, options = null) {
      ...
      this.mainPanel = html.div(null, 'JClicCustomMainPanel');
      this.gridPanel = html.div(null, 'JClicGridPanel');
      for (let i = 0; i < 9; i++)
        html.append(this.gridPanel, html.div(null, `JClicCell JClicCell${i + 1}`));
      html.append(this.mainPanel, this.gridPanel);
      this.playerCnt.remove();
      this.playerCnt.classList.add('JClicPlayerCell');
      this.mainPanel.appendChild(this.playerCnt);
      this.div.insertBefore(this.mainPanel, this.div.firstChild);
      ...
      if (options.buttons) {
        Object.keys(options.buttons.button).forEach(k => {
          ...
          this.buttons[k2] = html.element('button', null, `JClicBtn JClicTransform Btn-${k2}`, null, { title: msg, 'aria-label': msg, disabled: typeof this.msgKeys[k2] === 'undefined' });
          this.buttons[k2].addEventListener('click', evt => { if (ps.actions[k2]) ps.actions[k2].processEvent(evt); });
          this.mainPanel.appendChild(this.buttons[k2]);
        });
      }
      ...
      if (options.rectangle.messages) {
        ...
        this.msgBoxDiv.addEventListener('click', () => {
          this.msgBox.playMedia(ps);
          return false;
        });
        this.mainPanel.appendChild(this.msgBoxDiv);
      }
      ...
      if (false !== this.ps.options.counters && options.counters && options.counters.counter) {
        Object.keys(Skin.prototype.counters).forEach(name => {
          if (options.counters.counter[name]) {
            ...
            this.counters[name] = new Counter(name, html.div('000', `JClicCounter JClicTransform Counter-${name}`, { title: msg, 'aria-label': msg }));
            this.mainPanel.appendChild(this.counters[name].div);
          }
        });
      }
      ...
      if (options.progressAnimation) {
        this.progressAnimation = html.div(null, 'JClicProgressAnimation JClicTransform');
        this.mainPanel.appendChild(this.progressAnimation);
      }
    ...
    enableMainButtons(status) {
      this.mainPanel.querySelectorAll('.JClicBtn').forEach(btn => btn.setAttribute('tabindex', status ? '0' : '-1'));
    }
...
.ID .JClicTransform {transform: scale(0.666);}`;
    ...
    setWaitCursor(status) {
      ...
      if (this.progressAnimation)
        this.progressAnimation.style['animation-play-state'] = this.waitCursorCount > 0 ? 'running' : 'paused';
    }
