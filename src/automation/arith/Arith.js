        ...
        switch (child.nodeName) {
          case 'operand':
            switch (child.getAttribute('id')) {
              case 'A':
                this.opA.setProperties(child);
                break;
              case 'B':
                this.opB.setProperties(child);
                break;
            }
            break;
          case 'operations':
            this.use_add = Utils.getBoolean(child.getAttribute('plus'));
            this.use_subst = Utils.getBoolean(child.getAttribute('minus'));
            this.use_mult = Utils.getBoolean(child.getAttribute('multiply'));
            this.use_div = Utils.getBoolean(child.getAttribute('divide'));
            break;
          case 'unknown':
            this.exp_abx = Utils.getBoolean(child.getAttribute('result'));
            this.exp_xbc = Utils.getBoolean(child.getAttribute('first'));
            this.exp_axc = Utils.getBoolean(child.getAttribute('last'));
            this.exp_axbc = Utils.getBoolean(child.getAttribute('operand'));
            this.exp_caxb = Utils.getBoolean(child.getAttribute('inverse'));
            break;
          case 'result':
            xNum = child.getAttribute('from');
            this.resultLimInf = Utils.getNumber(xNum === 'x' ? 0 : xNum, this.resultLimInf);
            xNum = child.getAttribute('to');
            this.resultLimSup = Utils.getNumber(xNum === 'x' ? 0 : xNum, this.resultLimSup);
            this.resultCarry = Utils.getBoolean(child.getAttribute('notCarry'), this.resultCarry);
            this.resultNoDup = !Utils.getBoolean(child.getAttribute('duplicates'), !this.resultNoDup);
            let s = child.getAttribute('order');
            this.resultOrder = s === 'ascending' ? 'SORTASC' : s === 'descending' ? 'SORTDESC' : 'NOSORT';
            s = child.getAttribute('condition');
    ...
    setProperties(xml) {
      ...
      Utils.attrForEach(xml.attributes, (name, val) => {
        ...
        xml.childNodes.forEach(child => {
          switch (child.nodeName) {
            case 'include':
              this.wZero = Utils.getBoolean(child.getAttribute('zero'));
              this.wOne = Utils.getBoolean(child.getAttribute('one'));
              this.wMinusOne = Utils.getBoolean(child.getAttribute('minusOne'));
              break;
          }
        });
      });
